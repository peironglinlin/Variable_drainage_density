...
f_upa = "/scratch/gpfs/peirongl/MERIT/tif/upa/n30w090_upa.tif"
print('... reading '+f_upa+' ...')
f_dir = '/scratch/gpfs/peirongl/MERIT/tif/dir/'+f_upa[37:44]+'_dir.tif'
print('... reading '+f_dir+' ...')
ds1 = gdal.Open(f_upa)
ds2 = gdal.Open(f_dir)
...
rivIndices = np.where(dupa >= threshold)
ys = rivIndices[0]
xs = rivIndices[1]
nriv_grid = len(ys)
...
#     import pdb; pdb.set_trace()
df['node_flag']=0
df['node_flag'].loc[df['rivID'].isin(head)] = -1
df['node_flag'].loc[df['rivID'].isin(junction)] = range(1,len(junction)+1)
...
#         print(len(df1))
    ...
    dfh = pd.concat([dfh, df1])  #concatenate only for pixels before junctions
...
#df contains all river pixels with COMID assigned
